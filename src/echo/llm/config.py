...
class LLMConfig(BaseModel):
    ...
    temperature: float = float(_get_env_float("ECHO_DEFAULT_LLM_TEMPERATURE", 0.2))
    region: Optional[str] = os.getenv("AWS_DEFAULT_REGION") or None  # For Bedrock
    max_tokens: int = int(_get_env_int("ECHO_DEFAULT_LLM_MAX_TOKENS", 4096))
    # make this max of 10 calls not more than that
    max_iterations: int = int(_get_env_int("ECHO_DEFAULT_LLM_MAX_ITERATIONS", 10))
